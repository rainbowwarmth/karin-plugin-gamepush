...
export const zzzDownloadLinks = karin.command(`^#*${zzzReg}获取下载链接$`, async (e) => {
  try {
    ...
    const { msg, client, patch_client } = download.formatDownloadInfo("zzz", data, "main", patch) as {
      msg: string,
      client: string,
      patch_client: string
    }
...
export const zzzPreDownloadLinks = karin.command(`^#*${zzzReg}获取预下载链接$`, async (e) => {
  try {
    ...
    const { msg, client, patch_client } = download.formatDownloadInfo("zzz", data, "pre", patch) as {
      msg: string,
      client: string,
      patch_client: string
    }
