...
export const wwDownloadLinks = karin.command(`^#*${wwReg}获取下载链接$`, async (e) => {
  try {
    ...
    const { msg, client, patch_client } = download.formatDownloadInfo("ww", data, "main", patch) as {
      msg: string,
      client: string,
      patch_client: string
    }
...
export const wwPreDownloadLinks = karin.command(`^#*${wwReg}获取预下载链接$`, async (e) => {
  try {
    ...
    const { msg, client, patch_client } = download.formatDownloadInfo("ww", data, "pre", patch) as {
      msg: string,
      client: string,
      patch_client: string
    }
