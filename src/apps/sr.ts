...
export const srDownloadLinks = karin.command(`^#*${srReg}获取下载链接$`, async (e) => {
  try {
    ...
    const { msg, client, patch_client } = download.formatDownloadInfo("sr", data, "main", patch) as {
      msg: string,
      client: string,
      patch_client: string
    }
...
export const srPreDownloadLinks = karin.command(`^#*${srReg}获取预下载链接$`, async (e) => {
  try {
    ...
    const { msg, client, patch_client } = download.formatDownloadInfo("sr", data, "pre", patch) as {
      msg: string,
      client: string,
      patch_client: string
    }
