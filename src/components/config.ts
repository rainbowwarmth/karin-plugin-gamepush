...
class Config {
  ...
  init () {
    ...
    logger.error(`[karin-plugin-gamepush] 配置初始化失败`, err)
  ...
  static formatPushGroups (list: any[] = []) {
    return list
      .map((item: any) => {
  ...
  static serializePushGroups (list: any[] = []) {
    return list.map((item: any) => (typeof item === "string" ? item : `${item.botId}:${item.groupId}`))
  }
  ...
  loadConfig () {
    try {
      ...
      logger.error(`[karin-plugin-gamepush] 配置加载失败`, err)
      this.configCache = this.getDefaultConfig()
    }
  }
  ...
  removePushGroup (gameId: GameKey, botId: any, groupId: any) {
    this.updateGameConfig(gameId, (cfg) => {
  ...
  getFrontendConfig () {
    ...
    const frontendConfig: { [key: string]: any } = {}
    ...
    try {
      ...
      for (const gameId of gameIds) {
        saveData[gameId].pushGroups = [
          ...new Map(
            saveData[gameId].pushGroups.map((g: { botId: any; groupId: any }) => [`${g.botId}:${g.groupId}`, g])
          ).values()
        ]
      }
